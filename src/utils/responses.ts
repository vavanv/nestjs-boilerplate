const responses = {
  ...
  MESSAGE_INVALID_EMAIL_ADDRESS_OR_PASSWORD:
  ...
  MESSAGE_PASSWORD_UPDATED_SUCCESSFULLY:
    'Password has been updated successfully',
  MESSAGE_INVITE_LINK_EXPIRED: '',
  MESSAGE_OTP_SENT_SUCCESSFULLY: 'OTP Sent Successfully',
  MESSAGE_MAIL_SENT_SUCCESSFULLY: (param: string) =>
    `${param} mail has been sent successfully`,
  MESSAGE_CREATED: (collection_name: string) => {
    return `${collection_name} created successfully`;
  },
  MESSAGE_NOT_FOUND: (params: Array<string>) => {
    return `${params[0]} with this ${params[1]} is not found`;
  },
  ...
  MESSAGE_NOT_ACTIVE: (collection_name: string) => {
    return `${collection_name} is not active`;
  },
  MESSAGE_INVALID_FILE_TYPE: (allowed_file_types: Array<string>) => {
    return `Invalid File Type, allowed file types are: ${allowed_file_types}`;
  },
};
