...
export class TokenService {
  ...
  async create(insert_data: object, req): Promise<object> {
    try {
      ...
      return responses.get_response_object(
        ...
        { token: new_token },
        responses.MESSAGE_CREATED(TOKEN),
      );
    } catch (err) {
  ...
  async readOne(read_filter: FilterQuery<Token>): Promise<object> {
    try {
      ...
      this.winstonService.error(
        `path: 'reading one token',  data: ${JSON.stringify(
          read_filter,
        )},  message: ${err.message}`,
      );
      console.log(err.message);
  ...
  async updateOne(
    read_filter: FilterQuery<Token>,
    update_filter: UpdateQuery<Token>,
    query_options: QueryOptions<Token> = {},
  ): Promise<object> {
    try {
      const updated_token = await this.tokenModel.updateOne(
        read_filter,
        update_filter,
        query_options,
      );
      ...
      this.winstonService.error(
        `path: 'updating one token',  data: ${JSON.stringify({
          ...read_filter,
          ...update_filter,
        })},  message: ${err.message}`,
      );
  ...
  async updateMany(
    ...
    try {
      const updated_token = await this.tokenModel.updateMany(
        read_filter,
        update_filter,
        query_options,
      );
      ...
      this.winstonService.error(
        `path: 'updating many token',  data: ${JSON.stringify({
          ...read_filter,
          ...update_filter,
        })},  message: ${err.message}`,
      );
  ...
  async deleteMany(delete_filter: FilterQuery<Token>): Promise<object> {
    try {
      ...
      this.winstonService.error(
        `path: 'deleting many token',  data: ${JSON.stringify({
          delete_filter,
        })},  message: ${err.message}`,
      );
      console.log(err.message);
