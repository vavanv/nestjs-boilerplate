...
export class AuthService {
  ...
  async login(loginDto: LoginDto, req) {
    try {
      ...
      const existing_user = await this.usersService.readOne(
        { email_address: loginDto[EMAIL_ADDRESS] },
        req,
        { password: 0, password_salt: 0 },
      );
      if (existing_user[MESSAGE] != responses.MESSAGE_SUCCESS) {
        return responses.get_response_object(
          responses.CODE_NOT_FOUND,
          {},
          responses.MESSAGE_NOT_FOUND([USER, EMAIL_ADDRESS]),
        );
      }
      const match_password = common_utils.compare_password(
        loginDto[PASSWORD],
        existing_user[DATA].user.password,
      );
      if (!match_password) {
        return responses.get_response_object(
          responses.CODE_INVALID_CALL,
          {},
          responses.MESSAGE_INVALID_EMAIL_ADDRESS_OR_PASSWORD,
        );
      }
      const payload: JwtPayload = { email: loginDto[EMAIL_ADDRESS] };
      const encrypt_token: string = await common_utils.encrypt_token(
        payload.email,
      );
      const token = this.jwtService.sign(encrypt_token);
      await this.tokenService.create(
        {
          user: existing_user[DATA].user._id,
          token: token,
      ...
      return responses.get_response_object(
        ...
        { user: existing_user[DATA].user, token: token },
        responses.MESSAGE_SUCCESS,
      );
    } catch (err) {
      this.winstonService.error(
        `path: ${req.path},  data: ${JSON.stringify(loginDto)},  message: ${
          err.message
        }`,
      );
  ...
  async logout(token: string, req: any) {
    try {
      const existing_token = await this.tokenService.readOne({
        token: token,
        is_expired: false,
      });
      if (!existing_token) {
        return responses.get_response_object(
          responses.CODE_INVALID_CALL,
          {},
          responses.MESSAGE_INVALID_TOKEN,
        );
      }
      await this.tokenService.updateOne(
        ...
        {
          is_expired: true,
          expiry_time: common_utils.get_current_epoch_time(),
        },
      );
      return responses.get_response_object(
        responses.CODE_SUCCESS,
      ...
      this.winstonService.error(
        `path: ${req.path},  data: ${JSON.stringify({})},  message: ${
          err.message
        }`,
      );
      console.log(err.message);
