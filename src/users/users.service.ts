...
export class UsersService {
  ...
  async onModuleInit() {
    const [password, salt] = await common_utils.encrypt_password(
      environment.admin_password,
    );
    await this.userModel.findOneAndUpdate(
      { email_address: environment.admin_email },
      {
        email_address: environment.admin_email,
        password: password,
        password_salt: salt,
        name: environment.admin_name,
        phone_number: environment.admin_phone,
      },
      { upsert: true },
    );
  }
  ...
  async create(createUserDto: CreateUserDto, req): Promise<object> {
    try {
      createUserDto[EMAIL_ADDRESS] =
        createUserDto[EMAIL_ADDRESS].trim().toLowerCase();
      const existing_user = await this.userModel.exists({
        email_address: createUserDto[EMAIL_ADDRESS],
      });
      if (existing_user) {
        return responses.get_response_object(
          responses.CODE_ALREADY_EXISTS,
          {},
          responses.MESSAGE_ALREADY_EXISTS([USER, EMAIL_ADDRESS]),
        );
      }
      const [password, salt] = await common_utils.encrypt_password(
      ...
      const new_user = await this.userModel
        .findOne({ email_address: createUserDto[EMAIL_ADDRESS] })
        .select('-password -password_salt')
        .populate({
          path: 'created_by',
          select: 'name email_address phone_number display_picture',
      ...
      return responses.get_response_object(
        responses.CODE_CREATED,
        { user: new_user },
        responses.MESSAGE_CREATED(USER),
      );
    } catch (err) {
      this.winstonService.error(
        `status: ${req.status},  path: ${req.path},  data: ${JSON.stringify(
          createUserDto,
        )},  message: ${err.message}`,
      );
  ...
  async read(
    read_filter: FilterQuery<User> = {},
    projection_filter: QueryOptions<User> = {},
    req,
  ): Promise<object> {
    try {
      const users = await this.userModel
        .find(read_filter, {}, { projection: projection_filter })
        .populate({
          path: 'created_by',
          select: 'name email_address phone_number display_picture',
          options: { strictPopulate: false },
  ...
  async readOne(
    read_filter: FilterQuery<User> = {},
    req = null,
    projection_filter: object = {},
  ): Promise<object> {
    try {
      const user = await this.userModel
        .findOne(read_filter, {}, { projection: projection_filter })
        .populate({
          path: 'created_by',
          select: 'name email_address phone_number display_picture',
          options: { strictPopulate: false },
      ...
      if (!user)
        return responses.get_response_object(
          responses.CODE_NOT_FOUND,
          {},
          responses.MESSAGE_NOT_FOUND([USER, `${Object.keys(read_filter)[0]}`]),
        );
      return responses.get_response_object(
        responses.CODE_SUCCESS,
        { user: user },
  ...
  async update(
    read_filter: FilterQuery<User> = {},
    update_filter: UpdateQuery<User> = {},
    ...
    try {
      ...
      this.winstonService.error(
        `status: ${req.status},  path: ${req.path},  data: ${req.data},  message: ${err.message}`,
      );
      console.log(err.message);
      return responses.get_response_object(
        HttpStatus.BAD_REQUEST,
  ...
  async changePassword(
    ...
    try {
      ...
      return responses.get_response_object(
        responses.CODE_SUCCESS,
        { user: updated_user },
        responses.MESSAGE_PASSWORD_UPDATED_SUCCESSFULLY,
      );
    } catch (err) {
      this.winstonService.error(
        `status: ${req.status},  path: ${req.path},  data: ${JSON.stringify(
